...
class GeneticAlgorithm(ABC):
    ...
    def rank_population(self,):
        ...
        """
        fit_results = [(individual, self.calc_fitness(individual))
...
class MVPGeneticAlgorithm(GeneticAlgorithm):
    ...
    def init_population(self):
        ...
        for _ in range(self.pop_size):
            individual = np.hstack((np.array([0]), np.random.permutation(
                [i for i in range(1, self.num_pts)]), np.array([self.num_pts])))
    ...
    def calc_fitness(self, individual, show_log=False):
        ...
        cur_car_id = -1
        individual_cars = [{"cost": 0, "distance": 0, "demand": 0}
                           for _ in self.cars]
        empty_space = [-1 for _ in self.cars]
        full = False
        for i_r, j in enumerate(reversed(individual)):
            ...
            if j > len(self.custs):  # For car points
                cur_car_id = j
                weight = self.car_types[self.cars[cur_car_id -
                                                  1 - len(self.custs)]['type']]['cost']
                distance = self.dist_matrix[individual[i -
                                                       1] - 1, individual[i] - 1]
                assigned_cars[i] = cur_car_id
                total_cost += weight * distance
                total_distance += distance

                empty_space[cur_car_id - 1 - len(
                    self.custs)] = self.car_types[self.cars[cur_car_id - 1 - len(self.custs)]['type']]['capacity']

                individual_cars[cur_car_id - 1 -
                                len(self.custs)]['distance'] += distance
                individual_cars[cur_car_id - 1 -
                                len(self.custs)]['cost'] += weight * distance
            else:  # For customer points
                if (full or individual_cars[cur_car_id - 1 - len(self.custs)]['demand']+self.custs[j - 1]['demand']) >= self.car_types[self.cars[cur_car_id - 1 - len(self.custs)]['type']]['capacity']:
                    full = True
                    continue
                assert cur_car_id != -1
                weight = self.car_types[self.cars[cur_car_id -
                ...
                total_distance += distance
                individual_cars[cur_car_id - 1 -
                                len(self.custs)]['demand'] += self.custs[j - 1]['demand']
                individual_cars[cur_car_id - 1 -
                                len(self.custs)]['distance'] += distance
                individual_cars[cur_car_id - 1 -
                                len(self.custs)]['cost'] += weight * distance

                empty_space[cur_car_id - 1 -
                            len(self.custs)] -= self.custs[j - 1]['demand']

        if not invalid:

            fitness = np.exp(-0.05 * (total_cost + sum(empty_space)*5))
            ...
            if show_log:
                ...
                for i, j in enumerate(self.cars):
                    if individual_cars[i]['distance'] == 0:
                        continue
                    car_cnt += 1
                    print("Vehicle {0} (Type {1})".format(
                        car_cnt, chr(j['type']+ord('A'))))  # beautified counter
                    print("Round Trip Distance: {0:.3f} km, Cost: RM {1:.2f}, Demand: {2}".format(
                        individual_cars[i]['distance'], individual_cars[i]['cost'], individual_cars[i]['demand']))
                    print("Depot -> ", end='')
                    for ii, jj in enumerate(individual):
                        if assigned_cars[ii] != j['id']:
                            continue  # ignore those not belong to me
                        elif assigned_cars[ii] == jj:
                            print("Depot ({0:.3f} km)".format(
                                self.dist_matrix[individual[ii-1] - 1, individual[ii] - 1]))
                            break
                        print("C{0} ({1:.3f}km) -> ".format(
                            jj, self.dist_matrix[individual[ii-1] - 1, individual[ii] - 1]), end='')
                    print()
        else:
            if show_log:
                print("Infeasible.")
...
def gen_test_case(cust_size=30, rand_seed=42):
    ...
    for i in range(cust_size):
        custs.append({"id": 11+i, "pt": (4.3032+random.random(),
                     113.8322+random.random()), "demand": random.randint(5, 14)})
...
sample_test = {
    "path": r"C:\Users\tehyi\OneDrive - Asia Pacific University\Documents\CIO\tutorial\Sample Solution\sample_data.json",
    "best_ind": [],
    "best_score": 0,
    "avg_score": 0,
    "avg_time": 0
...
num_cars = 2
cars = [{"id": len(custs) + i + 1, "type": i // int(num_cars / 2)}
