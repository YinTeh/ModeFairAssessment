        ...
        for i_r, j in enumerate(reversed(individual)):
            ...
            if j > len(self.custs):  # For car points
                ...
                if individual_cars[cur_car_id - 1 - len(self.custs)]['demand'] > self.car_types[self.cars[cur_car_id - 1 - len(self.custs)]['type']]['capacity']:
                    # total demands of all the customers in this car exceed the maximum capacity, the individual is invalid hence fitness score is zero.
                    invalid = True
                    break

        if not invalid:
            fitness = np.exp(-0.05 * total_cost)
            if show_log:
